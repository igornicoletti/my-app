...
const isValidJoinOperator = (value: unknown): value is "and" | "or" =>
  value === "and" || value === "or"
...
export function DataTableFilterList<TData>({
  ...
  const [joinOperator, setJoinOperator] = React.useState<"and" | "or">(() => {
    try {
      const stored = window.localStorage.getItem(JOIN_OPERATOR_KEY)
      if (isValidJoinOperator(stored)) return stored
    } catch { }
    return "and"
  })
  ...
  React.useEffect(() => {
    try {
      window.localStorage.setItem(JOIN_OPERATOR_KEY, joinOperator)
    } catch { }
  }, [joinOperator])
  ...
  const onFilterAdd = React.useCallback(() => {
    ...
    debouncedSetFilters((currentFilters) => [
      ...currentFilters,
      {
  ...
  const onFilterRemove = React.useCallback(
    (filterId: string) => {
      setFilters((currentFilters) =>
        currentFilters.filter((filter) => filter.filterId !== filterId),
      )
    },
    [],
  )
