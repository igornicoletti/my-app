...
export function DataTableFilterMenu<TData>({
  ...
  const onFilterAdd = (column: Column<TData>, value: string) => {
  ...
  const onFilterRemove = (filterId: string) => {
    debouncedSetFilters(filters.filter((f) => f.filterId !== filterId))
  ...
  const onFilterUpdate = (
    ...
    debouncedSetFilters((prevFilters) =>
      prevFilters.map((f) =>
        f.filterId === filterId ? { ...f, ...updates } : f,
      ),
    )
  }
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <Popover open={open} onOpenChange={onOpenChange}>
        ...
        <PopoverContent align={align} className="w-[300px] p-0" {...props}>
