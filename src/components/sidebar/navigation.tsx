...
import type { Navigation } from '@/routes/config/navigation'
import { CaretRightIcon } from '@phosphor-icons/react'
import { Link, useLocation } from 'react-router-dom'

export const SidebarNavigation = ({ label, items }: Navigation) => {
              ...
              {!item.items || item.items?.length === 0 ? (
                ...
                <>
                  <CollapsibleTrigger asChild>
                    <SidebarMenuButton tooltip={item.title} isActive={item.items.some((sub) => sub.url === pathname)}>
                      {item.icon && <item.icon />}
                      <span>{item.title}</span>
